...
function push( version ) {
    if ( !git.isClean() ) {
        echo( 'Repo is not clean. Please add files and commit or stash your changes.' );
        cleanPropject();
        exit( 1 );
    }
    createSourceCopy();
    ...
    var publisher = new wp.Publisher(WP_REPO, version);
    publisher.addFiles( TEMP_DIR );
    publisher.commit(commitMessage);

    //wp.pushToWordPressOrg(
    //    WP_REPO,
    //    TEMP_DIR,
    //    commitMessage,
    //    version
